...
class HardwareConfig:
    ...
    Attributes:
        w_stage3: Bit width for Stage 3 (intra-block n-to-1 reduction)
        w_stage4: Bit width for Stage 4 (inter-block accumulation)
        w_reduce: Stage-3 fan-in (hardware groups this many K-groups per reduction; default 4)
    ...
    def __post_init__(self):
        ...
        if self.w_reduce < 2 or self.w_reduce > 16:
            raise ValueError(f"w_reduce should be in [2, 16], got {self.w_reduce}")
